...
class SectionAdd extends Command
{
     ...
     * @var string
     */
    protected $signature = 'make:section {name : The name of the sections} {template=coreui : The name of the template}';
    ...
    private function makeMenu()
    {
        ...
        if (File::exists(app_path('Http/Controllers/'.ucfirst($this->argument('name')).'/menu.php'))) {
            ...
            $data = File::get(__DIR__.'/stubs/menus.stub');
            $data = str_replace('{{{name}}}', strtolower($this->argument('name')), $data);
            $data = str_replace(
                '{{{controllerClass}}}',
                ucfirst($this->argument('name')).'\\Controllers\\Admin\\'.ucfirst($this->argument('name')).'Controller',
                $data
            );
    ...
    {
        $section = strtolower($this->argument('name'));

        $filePath = resource_path('lang/'.app()->getLocale().'/'.$section.'.php');

        if (File::exists($filePath)) {
            $this->error('Language file already exists.');
        } else {
            $data = File::get(__DIR__.'/stubs/section-lang.stub');
            $data = str_replace('{{{name}}}', $section, $data);
