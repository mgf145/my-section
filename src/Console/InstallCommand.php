...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'section:install {template=coreui : The name of the template}';
        ...
        if ($this->confirm('Do you want to create admin panel ? [y|n]', false)) {
            ...
            if ($this->confirm('Do you want to publish assets sources ? [y|n]', false)) {
                $this->call('vendor:publish', [
                    '--tag' => 'section-'.$this->argument('template').'-asset-sources',
                ]);
            }
        }
    ...
    {
        $this->call('vendor:publish', ['--tag' =>  'section-'.$this->argument('template').'-views']);
        $this->call('vendor:publish', ['--tag' =>  'section-'.$this->argument('template').'-assets']);
        ...
        if (!Str::contains($appConfig, 'Morilog\Jalali\Jalalian::class')) {
            ...
            foreach ($lines as $lineNumber => $line) {
                ...
                if (Str::contains($line, '\'aliases\'')) {
                    $appConfig .= /*$eol.*/"\t\t".'\'Jalalian\' => Morilog\Jalali\Jalalian::class,'/*.$eol*/;
                }
            }
